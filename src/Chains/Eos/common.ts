...
class EOS extends ChainInstance {
  ...
  async getTransactionInfo(txid: string): Promise<unknown> {
    ...
    if (!info || info?.error) { return { success: false, error: 'Error fetching tx info' } } const result = { success: true, account: info?.from, destination: info?.to, destinationTag: this.hexToStr(info?.input), amount: this.fromBaseUnit(info?.value) }
    return result
  }
